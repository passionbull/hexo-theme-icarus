...
module.exports = function (hexo) {
    ...
    hexo.extend.generator.register('archive', function(locals) {
      ...
      var list_updated_posts = hexo.extend.helper.get('list_updated_posts').bind(this);
      var result = [];
      ...
      function needs_update(posts) {
        if (config.incremental) {
          ...
          if (updated_posts && updated_posts.length > 0) {
            const files = posts["data"].map((p)=>(p["source"]))
            for (var f of updated_posts) {
